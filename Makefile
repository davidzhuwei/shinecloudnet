...
localnet-start: localnet-stop
	@if ! [ -f build/node0/scloud/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/scloud:Z tendermint/scloudnode testnet --v 4 -o . --starting-ip-address 10.10.10.2 --chain-id shinecloudnet-test; fi
	docker-compose up -d
